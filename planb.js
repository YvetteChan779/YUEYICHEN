(function () {
    ...
    const i18n = {
        en: {
            ...
            'hero.tagline': 'Full-stack voice-to-grasp pipeline on Jetson AGX Orin \u00b7 4 ASR engines \u00b7 12-mixin FSM \u00b7 3D grasp verification',
            'hero.report': 'Report',
            'hero.code': 'Code',
            'hero.demo': 'Demo',
            /* Metrics */
            ...
            'm.engines': 'ASR engine variants',
            'm.mixins': 'Pipeline FSM mixins',
            /* S0: Overview */
            's0.title': 'Overview',
            's0.p1': 'Deploying voice-controlled grasping on real robots exposes a critical gap: far-field speech is buried in noise, small objects defy segmentation models, and grasping attempts fail silently. <strong>PLAN-B</strong> is a full-stack voice-to-grasp pipeline deployed on Jetson AGX Orin. It chains a 3-tier VAD quality gate, a 4-engine streaming ASR router, dual-path NLU (regex + Qwen3-0.6B TRT-LLM), VLM bbox + CLIP text joint grounding with SAM3, and a 12-mixin closed-loop FSM with 3D multi-feature grasp verification.',
            ...
            's0.c3': 'Designed <strong>dual-path NLU</strong> (7-template regex fast-path + Qwen3-0.6B LLM, temperature=0, max_new_tokens=28) with a 5-layer semantic defense cascade, and <strong>VLM bbox + CLIP joint grounding</strong> with letterbox alignment, lifting SAM3 mask success 9% \u2192 \u226590%.',
            's0.c4': 'Built a <strong>12-mixin pipeline FSM</strong> with 3D multi-feature grasp verification (grasp_check v1.0.0: median<55mm, P25<45mm, frac<3cm\u22652%, frac<5cm\u22658%) and tactile baseline verification, enabling closed-loop retry with 4-class failure grading.',
            /* Demo */
            'sd.title': 'Demo',
            ...
            's1.cap': 'Full-stack pipeline on Jetson AGX Orin: voice frontend, semantic understanding, visual perception, closed-loop execution, and verification feedback.',
            's1.scope': '<strong>My scope:</strong> I owned the entire perception & integration layer \u2014 3-tier VAD, 4-engine streaming ASR, dual-path NLU, VLM-SAM3 visual grounding, 12-mixin closed-loop state machine, and 3D grasp verification. Downstream execution (navigation, IK planning, motor control) was handled by collaborators.',
            /* S2: Voice Frontend */
            ...
            /* S3: Streaming ASR */
            's3.title': 'Streaming ASR: 4-Engine Architecture',
            ...
            's3.e1.backend': 'WebSocket Realtime API (wss://dashscope)',
            's3.e1.audio': 'Raw passthrough + envelope AGC v5',
            's3.e1.eou': 'Server VAD (threshold=0.0, silence=1500ms)',
            's3.e2.backend': 'DashScope SDK (Recognition)',
            's3.e2.audio': 'RMS gate (280) + hangover (0.6s) + zero-fill',
            's3.e2.eou': 'VAD (max_sentence_silence=1800ms, heartbeat)',
            ...
            's5.h.mixin': '12-Mixin Pipeline Architecture',
        ...
        zh: {
            ...
            'hero.venue': '\u9999\u6e2f\u4e2d\u6587\u5927\u5b66',
            'hero.tagline': '\u5168\u6808\u8bed\u97f3\u5230\u6293\u53d6\u6d41\u6c34\u7ebf \u00b7 \u90e8\u7f72\u4e8e Jetson AGX Orin \u00b7 4 \u8def ASR \u5f15\u64ce \u00b7 12-Mixin \u72b6\u6001\u673a \u00b7 3D \u6293\u53d6\u9a8c\u8bc1',
            'hero.report': '\u62a5\u544a',
            'hero.code': '\u4ee3\u7801',
            'hero.demo': '\u6f14\u793a',
            /* Metrics */
            ...
            'm.mask': 'SAM3 \u63a9\u7801\u6210\u529f\u7387',
            'm.engines': 'ASR \u5f15\u64ce\u53d8\u4f53',
            'm.mixins': '\u7ba1\u7ebf FSM Mixin \u6570',
            /* S0: Overview */
            's0.title': '\u9879\u76ee\u6982\u8ff0',
            's0.p1': '\u5c06\u8bed\u97f3\u63a7\u5236\u6293\u53d6\u90e8\u7f72\u5230\u771f\u5b9e\u673a\u5668\u4eba\u4e0a\u66b4\u9732\u4e86\u4e00\u4e2a\u5173\u952e\u5dee\u8ddd\uff1a\u8fdc\u573a\u8bed\u97f3\u6df9\u6ca1\u5728\u566a\u58f0\u4e2d\uff0c\u5c0f\u76ee\u6807\u8ba9\u5206\u5272\u6a21\u578b\u5931\u8d25\uff0c\u6293\u53d6\u5c1d\u8bd5\u9759\u9ed8\u5931\u8d25\u3002<strong>PLAN-B</strong> \u662f\u90e8\u7f72\u5728 Jetson AGX Orin \u4e0a\u7684\u5168\u6808\u8bed\u97f3\u5230\u6293\u53d6\u7ba1\u7ebf\uff0c\u4e32\u8054 3 \u5c42 VAD \u8d28\u91cf\u95e8\u63a7\u30014 \u5f15\u64ce\u6d41\u5f0f ASR \u8def\u7531\u3001\u53cc\u8def\u5f84 NLU\uff08\u6b63\u5219 + Qwen3-0.6B TRT-LLM\uff09\u3001VLM bbox + CLIP \u6587\u672c\u8054\u5408\u5b9a\u4f4d\u4e0e SAM3\uff0c\u4ee5\u53ca 12-mixin \u95ed\u73af\u72b6\u6001\u673a\u4e0e 3D \u591a\u7279\u5f81\u6293\u53d6\u9a8c\u8bc1\u3002',
            ...
            's0.c3': '\u8bbe\u8ba1\u4e86<strong>\u53cc\u8def\u5f84 NLU</strong>\uff087 \u6a21\u677f\u6b63\u5219\u5feb\u8def\u5f84 + Qwen3-0.6B LLM\uff0ctemperature=0\uff0cmax_new_tokens=28\uff09\u914d\u5408 5 \u5c42\u8bed\u4e49\u9632\u7ebf\u7ea7\u8054\uff0c\u4ee5\u53ca <strong>VLM bbox + CLIP \u8054\u5408\u5b9a\u4f4d</strong>\u4e0e letterbox \u5bf9\u9f50\uff0cSAM3 \u63a9\u7801\u6210\u529f\u7387 9% \u2192 \u226590%\u3002',
            's0.c4': '\u6784\u5efa\u4e86<strong>12-mixin \u7ba1\u7ebf\u72b6\u6001\u673a</strong>\uff0c\u914d\u5408 3D \u591a\u7279\u5f81\u6293\u53d6\u9a8c\u8bc1\uff08grasp_check v1.0.0\uff1amedian<55mm\u3001P25<45mm\u3001frac<3cm\u22652%\u3001frac<5cm\u22658%\uff09\u548c\u89e6\u89c9\u57fa\u7ebf\u9a8c\u8bc1\uff0c\u5b9e\u73b0 4 \u7ea7\u5931\u8d25\u5206\u7c7b\u7684\u95ed\u73af\u91cd\u8bd5\u3002',
            /* Demo */
            'sd.title': '\u6f14\u793a',
            ...
            's1.scope': '<strong>\u6211\u7684\u804c\u8d23\uff1a</strong>\u8d1f\u8d23\u6574\u4e2a\u611f\u77e5\u4e0e\u96c6\u6210\u5c42 \u2014 3 \u5c42 VAD\u30014 \u5f15\u64ce\u6d41\u5f0f ASR\u3001\u53cc\u8def\u5f84 NLU\u3001VLM-SAM3 \u89c6\u89c9\u5b9a\u4f4d\u300112-mixin \u95ed\u73af\u72b6\u6001\u673a\u548c 3D \u6293\u53d6\u9a8c\u8bc1\u3002\u4e0b\u6e38\u6267\u884c\uff08\u5bfc\u822a\u3001IK \u89c4\u5212\u3001\u7535\u673a\u63a7\u5236\uff09\u7531\u534f\u4f5c\u56e2\u961f\u8d1f\u8d23\u3002',
            /* S2: Voice Frontend */
            ...
            's3.e2.eou': 'VAD \u65ad\u53e5\uff08max_sentence_silence=1800ms, heartbeat\uff09',
            ...
            's4.h.nlu': '\u53cc\u8def\u5f84\u610f\u56fe\u89e3\u6790',
            's4.p1': '<code>VoiceCommandParser</code> \u8fd0\u884c\u5728 <code>regex_then_llm</code> \u6a21\u5f0f\u3002<strong>\u5feb\u8def\u5f84</strong>\uff1a7 \u4e2a\u6b63\u5219\u6a21\u677f\u5728 <1 ms \u5185\u4ece\u6e05\u6670\u547d\u4ee4\u4e2d\u63d0\u53d6\u610f\u56fe\uff08pick_up / pick_and_place / place_only / move_to / chat / unknown\uff09\u548c\u5b9e\u4f53\u3002<strong>LLM \u8def\u5f84</strong>\uff1aQwen3-0.6B\uff08TRT-LLM \u6216 OpenAI \u517c\u5bb9 HTTP\uff09\uff0c\u7ed3\u6784\u5316\u7cfb\u7edf\u63d0\u793a\u5b9a\u4e49 6 \u79cd\u610f\u56fe\u7c7b\u578b\u3001\u566a\u58f0\u5206\u7c7b\u89c4\u5219\uff08\u4e71\u7801 ASR\u3001\u80cc\u666f\u8bed\u97f3\u3001\u586b\u5145\u77ed\u8bed\uff09\u548c\u5b9e\u4f53\u5b57\u6bb5\uff08target_object\u3001quantity\u3001location\u3001object_en\u3001location_en\uff09\u3002\u751f\u6210\u53c2\u6570\uff1a<code>temperature=0</code>\uff0c<code>max_new_tokens=28</code>\u3002<code>chat_fallback</code> \u6b63\u5219\u6355\u83b7 LLM \u5076\u5c14\u8bef\u5206\u7c7b\u4e3a unknown \u7684\u660e\u786e\u63d0\u95ee\u3002',
