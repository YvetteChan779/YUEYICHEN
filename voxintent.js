(function () {
    ...
    const i18n = {
        en: {
            ...
            /* JD Fit */
            'jd.title': 'Target Role & JD Fit',
            'jd.intro': 'This project is written against a specific target role in embodied-AI / robot voice interaction. The mapping below ties each job-description competency to concrete, code-traceable evidence in this pipeline. It is a fit argument, not a claim of novel research.',
            'jd.paste': '<strong>Job Description (paste here):</strong> the target posting is a WeChat article that is access-gated, so its text is not embedded automatically. Paste the JD text into <code>jd.jdtext</code> in <code>voxintent.js</code> to render it here verbatim. Source: <a href="https://mp.weixin.qq.com/s/fCcVl2ljaI3WZN-dVNCEOQ" target="_blank" rel="noopener">target posting</a>.',
            'jd.th.req': 'Typical JD Requirement',
            'jd.th.e0eng': 'VoxIntent Evidence',
            'jd.th.where': 'Where on this page',
            'jd.r1.req': 'Speech recognition / ASR integration for robots',
            'jd.r1.ev': 'Cloud-first 4-engine streaming ASR router (qwen3-asr-flash-realtime / paraformer-realtime-v2 / Qwen3-ASR TRT / FunASR), 150\u2013300 ms partials, cloud endpointing ownership, rapid-stitch.',
            'jd.r1.where': '<a href="#asr">Streaming ASR</a>',
            'jd.r2.req': 'Robust front-end audio in noisy / far-field conditions',
            'jd.r2.ev': '3-tier VAD (adaptive P10 floor + software AGC + stateful Silero v4 h/c [2,1,64]), envelope AGC v5, field calibration; far-field hit 0%\u219292%.',
            'jd.r2.where': '<a href="#vad">Voice Frontend</a>',
            'jd.r3.req': 'NLU / intent understanding & LLM integration',
            'jd.r3.ev': 'Dual-path NLU (6 action regex + qwen3.5-flash JSON, reasoning disabled), 6-state dialog manager with 3-score ambient rejection, 5-layer semantic defense.',
            'jd.r3.where': '<a href="#nlu">NLU & Dialog</a>',
            'jd.r4.req': 'Multimodal grounding (vision + language)',
            'jd.r4.ev': 'VLM instruction grounding with source-pixel xyxy bbox contract, letterbox alignment, color/category guards; box+text grounding 4/44\u2192\u226590%.',
            'jd.r4.where': '<a href="#vlm">VLM Grounding</a>',
            'jd.r5.req': 'On-device / edge deployment & latency engineering',
            'jd.r5.ev': 'Jetson AGX Orin deployment, HTTP connection pool + SSE early JSON, TTS echo guard (no AEC), circuit breaker; latency budgets stated per stage.',
            'jd.r5.where': '<a href="#tts">TTS Feedback</a> &middot; <a href="#results">Results</a>',
            'jd.r6.req': 'Full-pipeline / ROS2 system integration',
            'jd.r6.ev': 'ROS2 topic/service integration from ALSA capture to intent dispatch; explicit scope boundary vs collaborator-owned navigation, IK, and motor control.',
            'jd.r6.where': '<a href="#architecture">Pipeline Architecture</a>',
            'jd.caveat': '<strong>Honesty boundary:</strong> all metrics are field-reported on a single Jetson AGX Orin, not formal benchmarks. No ASR WER, NLU accuracy, or end-to-end task-success table is claimed. ASR/NLU/VLM/TTS models are off-the-shelf; the contribution is integration, calibration, and failure-cascade engineering.',
            /* S0: Overview */
            's0.title': 'Overview',
        ...
        zh: {
            ...
            'm.dict': 'zh\u2192en \u5bf9\u8c61\u8bcd\u5178',
            /* JD Fit */
            'jd.title': '\u76ee\u6807\u5c97\u4f4d\u4e0e JD \u5339\u914d',
            'jd.intro': '\u672c\u9879\u76ee\u9488\u5bf9\u5177\u4f53\u7684\u5177\u8eab\u667a\u80fd / \u673a\u5668\u4eba\u8bed\u97f3\u4ea4\u4e92\u76ee\u6807\u5c97\u4f4d\u64b0\u5199\u3002\u4e0b\u8868\u628a JD \u7684\u6bcf\u9879\u80fd\u529b\u8981\u6c42\u6620\u5c04\u5230\u672c\u7ba1\u7ebf\u4e2d\u53ef\u8ffd\u6eaf\u81f3\u4ee3\u7801\u7684\u8bc1\u636e\u3002\u8fd9\u662f\u5339\u914d\u8bba\u8bc1\uff0c\u800c\u975e\u65b0\u9896\u7814\u7a76\u58f0\u660e\u3002',
            'jd.paste': '<strong>Job Description\uff08\u8bf7\u7c98\u8d34\uff09\uff1a</strong>\u76ee\u6807\u5c97\u4f4d\u4e3a\u5fae\u4fe1\u6587\u7ae0\uff0c\u5b58\u5728\u8bbf\u95ee\u9a8c\u8bc1\u95e8\u63a7\uff0c\u5176\u6b63\u6587\u65e0\u6cd5\u81ea\u52a8\u5d4c\u5165\u3002\u8bf7\u5c06 JD \u6587\u672c\u7c98\u8d34\u5230 <code>voxintent.js</code> \u7684 <code>jd.jdtext</code> \u4e2d\u4ee5\u9010\u5b57\u6e32\u67d3\u3002\u6765\u6e90\uff1a<a href="https://mp.weixin.qq.com/s/fCcVl2ljaI3WZN-dVNCEOQ" target="_blank" rel="noopener">\u76ee\u6807\u5c97\u4f4d\u94fe\u63a5</a>\u3002',
            'jd.th.req': 'JD \u5178\u578b\u8981\u6c42',
            'jd.th.e0eng': 'VoxIntent \u8bc1\u636e',
            'jd.th.where': '\u672c\u9875\u5bf9\u5e94\u4f4d\u7f6e',
            'jd.r1.req': '\u9762\u5411\u673a\u5668\u4eba\u7684\u8bed\u97f3\u8bc6\u522b / ASR \u96c6\u6210',
            'jd.r1.ev': '\u4e91\u4f18\u5148 4 \u5f15\u64ce\u6d41\u5f0f ASR \u8def\u7531\uff08qwen3-asr-flash-realtime / paraformer-realtime-v2 / Qwen3-ASR TRT / FunASR\uff09\uff0c150\u2013300 ms partial\uff0c\u4e91\u7aef endpointing \u4e3b\u6743\uff0c\u5feb\u901f\u62fc\u63a5\u3002',
            'jd.r1.where': '<a href="#asr">\u6d41\u5f0f ASR</a>',
            'jd.r2.req': '\u566a\u58f0 / \u8fdc\u573a\u6761\u4ef6\u4e0b\u7684\u9c81\u68d2\u524d\u7aef\u97f3\u9891',
            'jd.r2.ev': '3 \u5c42 VAD\uff08\u81ea\u9002\u5e94 P10 \u5e95 + \u8f6f\u4ef6 AGC + \u6709\u72b6\u6001 Silero v4 h/c [2,1,64]\uff09\u3001\u5305\u7edc AGC v5\u3001\u73b0\u573a\u6807\u5b9a\uff1b\u8fdc\u573a\u547d\u4e2d 0%\u2192 92%\u3002',
            'jd.r2.where': '<a href="#vad">\u8bed\u97f3\u524d\u7aef</a>',
            'jd.r3.req': 'NLU / \u610f\u56fe\u7406\u89e3\u4e0e LLM \u96c6\u6210',
            'jd.r3.ev': '\u53cc\u8def\u5f84 NLU\uff086 \u4e2a\u52a8\u4f5c\u6b63\u5219 + qwen3.5-flash JSON\uff0c\u7981\u7528 reasoning\uff09\u30016 \u72b6\u6001\u5bf9\u8bdd\u7ba1\u7406\u5668\u642d\u914d 3 \u7ef4\u5ea6\u73af\u5883\u97f3\u62d2\u7edd\u30015 \u5c42\u8bed\u4e49\u9632\u7ebf\u3002',
            'jd.r3.where': '<a href="#nlu">NLU \u4e0e\u5bf9\u8bdd</a>',
            'jd.r4.req': '\u591a\u6a21\u6001\u5b9a\u4f4d\uff08\u89c6\u89c9 + \u8bed\u8a00\uff09',
            'jd.r4.ev': 'VLM \u6307\u4ee4\u5b9a\u4f4d\uff0c\u539f\u56fe\u50cf\u7d20 xyxy bbox \u5951\u7ea6\u3001letterbox \u5bf9\u9f50\u3001\u989c\u8272/\u7c7b\u522b\u62a4\u680f\uff1bbox+text \u5b9a\u4f4d 4/44\u2192 \u226590%\u3002',
            'jd.r4.where': '<a href="#vlm">VLM \u5b9a\u4f4d</a>',
            'jd.r5.req': '\u7aef\u4fa7 / \u8fb9\u7f18\u90e8\u7f72\u4e0e\u5ef6\u8fdf\u5de5\u7a0b',
            'jd.r5.ev': 'Jetson AGX Orin \u90e8\u7f72\u3001HTTP \u8fde\u63a5\u6c60 + SSE \u65e9\u671f JSON\u3001TTS \u56de\u58f0\u95e8\u63a7\uff08\u65e0 AEC\uff09\u3001\u7194\u65ad\u5668\uff1b\u9010\u7ea7\u5ef6\u8fdf\u9884\u7b97\u660e\u786e\u3002',
            'jd.r5.where': '<a href="#tts">TTS \u53cd\u9988</a> &middot; <a href="#results">\u7ed3\u679c</a>',
            'jd.r6.req': '\u5168\u94fe\u8def / ROS2 \u7cfb\u7edf\u96c6\u6210',
            'jd.r6.ev': '\u4ece ALSA \u91c7\u96c6\u5230\u610f\u56fe\u5206\u53d1\u7684 ROS2 topic/service \u96c6\u6210\uff1b\u4e0e\u534f\u4f5c\u65b9\u62e5\u6709\u7684\u5bfc\u822a\u3001IK\u3001\u7535\u673a\u63a7\u5236\u660e\u786e\u5212\u5206\u8fb9\u754c\u3002',
            'jd.r6.where': '<a href="#architecture">\u7ba1\u7ebf\u67b6\u6784</a>',
            'jd.caveat': '<strong>\u8bda\u5b9e\u8fb9\u754c\uff1a</strong>\u6240\u6709\u6307\u6807\u5747\u4e3a\u5355\u53f0 Jetson AGX Orin \u73b0\u573a\u62a5\u544a\uff0c\u975e\u6b63\u5f0f benchmark\u3002\u4e0d\u58f0\u79f0 ASR WER\u3001NLU \u51c6\u786e\u7387\u6216\u7aef\u5230\u7aef\u4efb\u52a1\u6210\u529f\u7387\u8868\u3002ASR/NLU/VLM/TTS \u6a21\u578b\u5747\u4e3a\u73b0\u6210\uff1b\u8d21\u732e\u5728\u4e8e\u96c6\u6210\u3001\u6807\u5b9a\u4e0e\u6545\u969c\u7ea7\u8054\u5de5\u7a0b\u3002',
            /* S0: Overview */
            's0.title': '\u9879\u76ee\u6982\u8ff0',
